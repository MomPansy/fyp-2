...
const studentAssessments = pgTable(
  "student_assessments",
  {
    ...
    student: uuid("student_id").notNull().references(() => users.id, { onDelete: "cascade" })
  },
  (table) => [
...
const studentAssessmentPolicy = pgPolicy("student_role_policy", {
  as: "permissive",
  for: "all",
  to: ["public"],
  using: sql`EXISTS (
...
const studentAssessmentRelations = relations(
  studentAssessments,
  ({ one }) => ({
    user: one(users, {
